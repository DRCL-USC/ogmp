...
import matplotlib.ticker as ticker
              ...
              #z,wr,wp,wy,vx,vy,vz
Q_z = np.diag([11,1,1,1,10,3,0])
Q_vz =np.diag([0,1,1,1,1,3,3])
...
class oracle_var(oracle):
    ...
    def get_traj(self, mode_name, input):
        ...
        c_mat = np.zeros((Oplen,13))
        
        for k,key in enumerate(self.ref_list):
            if k ==0:
                y_ref = state_ref[self.ref_list[key]]
            else:
                y_ref = np.vstack((y_ref,state_ref[self.ref_list[key]]))
            c_mat[k,self.ref_list[key]] = 1
        ...
        for k in range(N+1):
            ...
            if k :
                if bool(contact_ref[k])^bool(contact_ref[k-1]) :      # condition for switch 
                    ...
                    lpc_obj.compute_params(a_mat,b_mat,c_mat,q_mat,R,H_nom)
                    i+=1
            ...
            #stance phase 
            if contact_ref[k] ==1:
                control_vector = lpc_obj.get_control(state_vector,y_ref[:,k:k+H_nom])
...
if __name__ == "__main__":
    ...
    internal_params = {'hi':0,'hf':0.0,'x0':x_init,"delta_apos":np.array([[0],[0],[2*np.pi]])}
