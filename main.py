...
if __name__ == "__main__":
  ...
  if len(sys.argv) < 2:
    ...
    print("\t potential options are: 'ppo', 'eval_tstng'")
    exit(1)

  option = sys.argv[1]
  sys.argv.remove(sys.argv[1])

  # to evaluate and test policy(s)
  if option == 'eval_tstng':
    from util import eval_policy_tstg

    parser.add_argument("--trng_conf",          action='store_true')
